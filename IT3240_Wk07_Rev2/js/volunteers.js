...
const displayVolunteers = function () {   
    ...
    // Building a numbered list of volunteers 
    let volunteerlist = "";

    // Declare and initialize counter variable to 1 (value for number before name)
    //declaring variable counter to keep track of numbering
    let counter = 1;
    ...
    for (let i = 0; i < volunteerArray.length; i++) {
        volunteerlist += counter + ". " + volunteerArray[i] + "\n";
        counter++;
    }
    findById("volunteer-list-area").value = volunteerlist;
...
const deleteVolunteer = function () {
	...
	for (let i = 0; i < volunteerArray.length; i++) {
        if (volunteerArray[i] === volunteerString) {
            ...
            break; // Exiting the loop after deleting the first matching entry
        }
    }
